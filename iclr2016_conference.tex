...
In the training data, 62.8\% of the known data points are positive, (That value is 57.6\% for the
testing data.) Due to this imbalance, we do not use raw accuracy as a predictor and instead turn to
the Receiver Operating Characteristic curve. Fig~\ref{fig:mooc_accuracy} shows the ROC curve.

\textbf{Daniel: OK, I updated the ROC plot. I should also mention that I ran this for 1000 passes,
and the AUC for that is 0.63792, so I did not include that. (It might be overfitting, or that might
be the theoretical limit.) Also, I'll need to explain what I mean by the percentage points on the
red circles.}
...
\section{Scaling BIDMach to Larger Data}\label{sec:scaling_large_data}
