...
same $KL_{\rm avg}$ metric from~(\ref{ssec:student_data}), since we know the CPT that generated the
data.
...
thousands of iterations, the $m=10$ curve was essentially neck-to-neck with the $m=1$ curve, but it
is much slower and had similar variance.

\textbf{Daniel: Note that we are planning on including one JAGS curve ($m=1$) in
Figure~\ref{fig:mooc_kl}. We will need to explicitly state the runtimes as well (and also describe
the runtime for BIDMach). Also, I should explain the ``long term'' trend stuff better, perhaps even
...
into a training and testing set, which we do by holding out one mini-batch for testing. Then, using
our data, we ... \textbf{Daniel: TODO I am trying to think about the easiest way to test.}
...
\textbf{TODO I would like to do this, and I think this is important. We should be seeing that
accuracy goes up to about 66\%.  Unfortunately, the actual data is skewed in a 66\% to 33\% ratio so
... let me figure out if I can spin that positively.}
