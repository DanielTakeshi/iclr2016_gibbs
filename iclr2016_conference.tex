...
for different $m$ values, on the Koller data with one million instances. For each trial, we ran 200
...
% Daniel: more experiments on Veteran's day.
%
% I'm running this again w/the same settings (i.e., all on the GPU). This should be very similar:
%          & $m=1$ & $m=5$  & $m=10$ & $m=20$ & $m=30$  & $m=40$  & $m=50$  \\
% \hline \hline
% GigaFlops & 2.42  & 7.60   & 10.06  & 12.29  & 13.31  & 13.80  & 13.95   \\ 
% Runtime   & 67.38 & 107.22 & 162.03 & 265.39 & 367.42 & 472.50 & 584.46  \\
%
% Now here it is, but WITH CPU GAMMA RANDOM variables, but GPU otherwise!
%          & $m=1$ & $m=5$  & $m=10$ & $m=20$ & $m=30$  & $m=40$  & $m=50$  \\
% \hline \hline
% GigaFlops & 2.49  & 7.74   & 10.18  & 12.40  & 13.42  & 13.84  & 13.93   \\ 
% Runtime   & 65.38 & 105.33 & 160.15 & 263.09 & 364.64 & 471.37 & 585.12  \\
%
% This is interesting. I thought switching from GPU-CPU was expensive, but it seems to be faster?
...
\section{Actual Tables}

(Draft section)

These are the set of \emph{actual} tables we will be using. Let's clean this up later once we have
better figures. Also, I'll keep more detailed results in LaTeX comments.
...
%
% Here are the BIDMach CPU (not GPU!) results, with extra values if we can get more JAGS results:
...
%
% Yeah, a  bit weird that gflops doesn't increase each time ...
\begin{table}[h]
...
\hline
                  & $m=1$ & $m=5$ & $m=10$ \\
\hline \hline
BIDMach Time/Iter & 0.044 & 0.149 & 0.338  \\ 
JAGS Time/Iter    & & & \\
\hline
\end{tabular}
\end{center}
...
%
% Here are the BIDMach CPU (not GPU!) results:
%
...
% Note: I suppose I should run this with 60% data as well...
%
...
\hline
                  & 1x    & 2x    & 5x    & 10x   & 20x \\
\hline \hline
BIDMach Time/Iter & 0.182 & 0.358 & 0.983 & 1.936 & 3.699 \\ 
JAGS Time/Iter    & & & & & \\
\hline
\end{tabular}
\end{center}
